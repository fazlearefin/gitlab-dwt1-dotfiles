...
#+begin_src emacs-lisp
;; Magit is awesome but it doesn't work well with git bare repos that many people use to maintain there dotfiles
;; this is wrapper around the maggit-status command to use with git bare repos
