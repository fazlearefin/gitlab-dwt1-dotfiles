...
#+begin_src emacs-lisp
;; set-face-attribute does't work until the frame is created so it doesn't work in emacsclient with a daemon running
;; this function will set correct font regardless of emacs daemon's status
(defun me/set-font ()
...
;; if in daemon is running set fonts after frame is made 
(if (daemonp)
...
#+begin_src emacs-lisp
;; Magit is awesome but it doesn't work well with git bare repos many people use to maintain there dotfiles
;; this wrapper around the maggit status to use with git bare repos set appropriate file path and working tree for your config 
;; keybind this to manage your git bare repos like dotfiles repos with magit don't forget to set appropriate bare-git-dir and bare-work-tree for your config
;; keybind this
...
(defun me/magit-status- ()
  ...
  (interactive)
  (require 'magit-git)
  (let ((magit-git-global-arguments (append magit-git-global-arguments (list bare-git-dir bare-work-tree))))
