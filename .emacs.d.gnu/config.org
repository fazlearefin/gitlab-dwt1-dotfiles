...
* EXWM
  a emacs x window manager
#+begin_src emacs-lisp
  ;; -*- lexical-binding: t; -*

  (defun me/exwm-set-modeline ()
    "sets up a modeline for exwm"
    (progn
  ...
  (defun me/exwm-spawn(name command)
  "spawn application"
    (interactive)
    (start-process-shell-command name nil command))

  (defun me/exwm-spawn-in-background (command)
  "spawn application in background"
    (let ((command-parts (split-string command "[ ]+")))
      (apply #'call-process `(,(car command-parts) nil 0 nil ,@(cdr command-parts)))))

  (defun me/exwm-toggle-minibuffer ()
  ...
  (defcustom me/skippable-buffer-regexp
    (rx bos (or "*Messages*" "*scratch*" "*Help*" "*Backtrace*") eos)
    "Matching buffer names are ignored by `me/next-buffer'
  and `me/previous-buffer'."
    :type 'regexp)

  (defun me/change-buffer (change-buffer)
    "Call CHANGE-BUFFER until `me/skippable-buffer-regexp' doesn't match."
    (let ((initial (current-buffer)))
      (funcall change-buffer)
      (let ((first-change (current-buffer)))
        (catch 'loop
          (while (string-match-p me/skippable-buffer-regexp (buffer-name))
            (funcall change-buffer)
            (when (eq (current-buffer) first-change)
              (switch-to-buffer initial)
              (throw 'loop t)))))))

  (defun me/next-buffer ()
    "Variant of `next-buffer' that skips `me/skippable-buffer-regexp'."
    (interactive)
    (me/change-buffer 'next-buffer))

  (defun me/previous-buffer ()
    "Variant of `previous-buffer' that skips `me/skippable-buffer-regexp'."
    (interactive)
    (me/change-buffer 'previous-buffer))

  (defun me/switch-next-buffer-or-window ()
  ...
  (defun me/switch-prev-buffer-or-window ()
  ...
  ;; backlight
  (defun me/exwm-backlight-ctl (opt)
  ...
  ;; screenshot
  ;; this requries maim
  (setq me/screenshot-dir "~/Pictures")
  (defun me/exwm-screenshot (&optional rename select)
  ...
  (let ((system-executables (seq-mapcat
                             ...
                             exec-path)))
    (defun me/exwm-run-prompt (command)
  ...
  (let ((system-power-options
    ...
    (defun me/exwm-power-prompt(command)
  ...
  ;; package manager
  (defun me/exwm-pacman-install-prompt (command)
  ...
  (defun me/exwm-pacman-uninstall-prompt (command)
  ...
  (defun me/exwm-paru-install-prompt (command)
    "a prompt to install packages"
  ...
  (use-package exwm
    ...
    ;;(add-hook 'exwm-manage-finish-hook #'me/exwm-hide-modeline-and-minibuffer)

    ;; rename buffers
    (add-hook 'exwm-update-class-hook #'me/exwm-rename-buffer)
    (add-hook 'exwm-update-title-hook #'me/exwm-rename-buffer)

    ;; window rules
    (add-hook 'exwm-manage-finish-hook 'me/exwm-window-rules)
    ...
    ;; window split
    (exwm-input-set-key (kbd "s-\\") #'me/split-window-right)
    (exwm-input-set-key (kbd "s-|") #'me/split-window-below)
    (exwm-input-set-key (kbd "s-=") #'balance-windows)
    (exwm-input-set-key (kbd "s-n") #'me/toggle-single-window)
    (exwm-input-set-key (kbd "s-N") #'me/toggle-window-split)

    ;; window focus switching
    (exwm-input-set-key (kbd "s-j") #'me/switch-next-buffer-or-window)
    (exwm-input-set-key (kbd "s-k") #'me/switch-prev-buffer-or-window)
    ...
    (exwm-input-set-key (kbd "s-M") #'me/exwm-toggle-minibuffer)
    ...
    (exwm-input-set-key (kbd "s-F") (lambda() (interactive) (me/exwm-spawn "pcmanfm" "pcmanfm")))
    (exwm-input-set-key (kbd "s-T") (lambda() (interactive) (me/exwm-spawn "xterm" "xterm")))
    (exwm-input-set-key (kbd "s-b") (lambda() (interactive) (me/exwm-spawn "chromium" "chromium --disable-software-rasterizer --profile-directory='Profile 1'")))
    (exwm-input-set-key (kbd "s-B") (lambda() (interactive) (me/exwm-spawn "chromium" "chromium --disable-software-rasterizer --profile-directory='Profile 2'")))

    ;; application launcher
    (exwm-input-set-key (kbd "s-SPC") #'me/exwm-run-prompt)
    ...
    ;; system prompts
    (exwm-input-set-key (kbd "s-s p") #'me/exwm-power-prompt)
    (exwm-input-set-key (kbd "s-s m") #'man)
    (exwm-input-set-key (kbd "s-s h") #'proced)

    ;; pacman prompts
    (exwm-input-set-key (kbd "s-P i") #'me/exwm-pacman-install-prompt)
    (exwm-input-set-key (kbd "s-P r") #'me/exwm-pacman-uninstall-prompt)
    (exwm-input-set-key (kbd "s-P u") (lambda() (interactive) (start-process-shell-command "pacman update" nil "xterm -e sudo pacman -Syu")))
    (exwm-input-set-key (kbd "s-P y") #'me/exwm-paru-install-prompt)

    ;; volume control
    (exwm-input-set-key (kbd "<XF86AudioRaiseVolume>") (lambda () (interactive) (me/exwm-volume-ctl "5%+")))
    (exwm-input-set-key (kbd "<XF86AudioLowerVolume>") (lambda () (interactive) (me/exwm-volume-ctl "5%-")))
    (exwm-input-set-key (kbd "<XF86AudioMute>")        (lambda () (interactive) (me/exwm-volume-ctl "toggle")))

    ;; backlight control
    (exwm-input-set-key (kbd "<XF86MonBrightnessUp>")   (lambda () (interactive) (me/exwm-backlight-ctl "-inc +2")))
    (exwm-input-set-key (kbd "<XF86MonBrightnessDown>") (lambda () (interactive) (me/exwm-backlight-ctl "-dec +2")))

    ;;screenshot
    (exwm-input--set-key (kbd "<print>") (lambda () (interactive) (me/exwm-screenshot nil nil)))
    (exwm-input--set-key (kbd "S-<print>") (lambda () (interactive) (me/exwm-screenshot nil t)))
    (exwm-input--set-key (kbd "s-<print>") (lambda () (interactive) (me/exwm-screenshot t nil)))
    (exwm-input--set-key (kbd "s-S-<print>") (lambda () (interactive) (me/exwm-screenshot t t)))
